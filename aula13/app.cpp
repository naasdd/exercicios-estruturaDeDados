...
// Caso LR: filho esquerdo ta pesado pra direita
// rotacao esquerda no filho, depois direita na raiz
No* rotacaoDuplaDireita(No* noAtual) {
    noAtual->esquerdo = rotacaoEsquerda(noAtual->esquerdo);
    return rotacaoDireita(noAtual);
}

// Caso RL: filho direito ta pesado pra esquerda
// rotacao direita no filho, depois esquerda na raiz
No* rotacaoDuplaEsquerda(No* noAtual) {
    noAtual->direito = rotacaoDireita(noAtual->direito);
...
No* balancear(No* no) {
    ...
    if (fb > 1 && fatorBalanceamento(no->esquerdo) >= 0)
        return rotacaoDireita(no);          // LL

    if (fb > 1 && fatorBalanceamento(no->esquerdo) < 0)
        return rotacaoDuplaDireita(no);     // LR

    if (fb < -1 && fatorBalanceamento(no->direito) <= 0)
        return rotacaoEsquerda(no);         // RR

    if (fb < -1 && fatorBalanceamento(no->direito) > 0)
        return rotacaoDuplaEsquerda(no);    // RL
...
int main() {
    ...
    avl = inserirNo(avl, 40);
    avl = inserirNo(avl, 30);
    avl = inserirNo(avl, 60);
    avl = inserirNo(avl, 45);
    avl = inserirNo(avl, 47);
    avl = inserirNo(avl, 43);
    
    cout << "Em ordem:  "; emOrdem(avl);  cout << endl;
    cout << "Pre ordem: "; preOrdem(avl); cout << endl;
    cout << "Altura da raiz: " << altura(avl) << endl;

    encontrado = buscar(avl, 20);
    cout << "Busca por 25: " << (encontrado ? "encontrado" : "nao encontrado") << endl;
